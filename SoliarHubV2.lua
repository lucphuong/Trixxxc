...
if World1 or World2 or World3 then
    Tabs.Dungoen:AddSection("Join Dungoen To Unlock Function")
end
if dungeon then
    local AutoDungeonRaid = Tabs.Dungeon:AddToggle("AutoDungeonRaid", {
        Title = "Auto Dungeon Raid",
        Default = false
    })
AutoDungeonRaid:OnChanged(function(value)
        AutoDungeon = value
    end)
end
task.spawn(function()
    while task.wait(sec) do
        if AutoDungeon then
            ...
            if hrp then
                ...
                if nearestMob and _tp then
                    _tp(nearestMob.HumanoidRootPart.CFrame * CFrame.new(0, 0, -3))
                end
            end
        end
    end
